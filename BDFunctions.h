...
#define MASTER_DATABASE "master_data.fl"
#define MASTER_GARBAGE "master_garbage.txt"
#define SLAVE_DATABASE "slave_data.fl"
#define SLAVE_GARBAGE "slave_garbage.txt"
#define MASTER_INDEXER_DB "master_indexer.ind"

//MASTER FUNCTIONS
int DeleteSlave(struct Master master, struct Slave slave, int id);
void MarkDeletedMaster(int id)// записує id Master якого видалили
{
    FILE* masterGarbage = fopen(MASTER_GARBAGE, "rb");
    int garbageCount;
    fscanf(masterGarbage, "%d", &garbageCount);
    int* deletedMasterIds = malloc(garbageCount * sizeof(int));
...
int InsertMaster(struct Master master)
{
    ...
    if (garbageCount > 0)
    {
        RewriteMasterGarbageId(&master, masterGarbage, garbageCount);
...
{
    long* deletedSlaveIds = malloc(garbageCount * sizeof(long));
    for (int i = 0; i < garbageCount; i++)
    {
